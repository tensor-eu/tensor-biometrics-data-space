...
public class SelfDescriptionManager {
  ...
  /**
   * Adds resource to Offered resources in Resource Catalog with resourceCatalogId
  ...
  public Connector addOfferedResource(Connector connector, URI resourceCatalogId, Resource resource) {
    ...
    logger.info("ADD RESOURCE KAT {}", resource);
  ...
  public Connector addRepresentationToResource(Connector connector, Representation representation, URI resourceId) {
    ...
    if (!representationExists) {
      ...
      throw new BadRequestException(
          String.format("Representation with id '%s' already exists", representation.getId()));
    }
    return connector;
  }
  ...
  public Connector updateRepresentationToResource(Connector connector, Representation representation, URI resourceId) {
    ...
    if (representationExists) {
      ...
      throw new ResourceNotFoundException(
          String.format("Representation with id '%s' does not exist", representation.getId()));
    }
    return connector;
  }
  ...
  public Connector updateContractOfferToResource(Connector connector, ContractOffer contractOffer, URI resourceId) {
    ...
    if (contractOfferExists) {
      ...
      throw new ResourceNotFoundException(
          String.format("Contract offer with id '%s' does not exist", contractOffer.getId()));
    }
    return connector;
  }
   ...
   * at least one representation with at least one artifact and at least one
   * contract offer
   * 
   * @param connector Base Connector
  ...
  public void saveConnector() {
    ...
    try {
      String connectorAsString = MultipartMessageProcessor
          .serializeToJsonLD(SelfDescription.getInstance().getConnector());
  ...
  public Connector loadConnector() {
    ...
    if (selfDescriptionFile.exists() && !selfDescriptionFile.isDirectory()) {
      ...
      try {
        ...
        logger.info("Either delete '{}' and get default self description document or try to fix the error",
            selfDescriptionFile.getAbsoluteFile());
